...
impl ScaleTarget {
    /// The spelling recorded in the output, which is also what `--scale` accepts.
    ...
    /// The inverse of [`Self::as_str`]. An unknown or absent spelling reads as `All`, so a row
    /// written before this field existed still labels itself the way it ran.
    pub(crate) fn named(spelling: &str) -> Self {
        [Self::All, Self::Vaults, Self::Ladder]
            .into_iter()
            .find(|target| target.as_str() == spelling)
            .unwrap_or_default()
    }
...
impl ArmSpec {
    ...
    fn effect(&self) -> (u64, u64, u64) {
        ...
        (bits(self.capital()), bits(self.depth()), bits(self.tighten_bps))
    }
...
pub(crate) struct Cli {
    ...
    /// Per-arm output.
    #[arg(long, default_value = "counterfactual-capital.jsonl", value_name = "PATH")]
...
impl Cli {
    ...
    pub(crate) fn arms(&self) -> Result<Vec<ArmSpec>> {
        ...
        arms.sort_by(|a, b| {
            b.is_control()
                .cmp(&a.is_control())
                .then_with(|| a.multiple.partial_cmp(&b.multiple).expect("finite"))
                .then_with(|| a.target.as_str().cmp(b.target.as_str()))
                .then_with(|| a.tighten_bps.partial_cmp(&b.tighten_bps).expect("finite"))
        });
        // Every target collapses to the same bytes at a multiple of one.
        arms.dedup_by(|a, b| a == b);
        ensure!(
            arms.len() <= MAX_ARMS,
...
mod tests {
    ...
    fn scaled(multiples: &[f64], tighten: &[f64], scales: &[&str]) -> Cli {
        Cli::parse_from(
            ["cf"]
                .into_iter()
                .map(String::from)
                .chain(["--api-key=k".to_string(), "--start-slot=1".to_string(), "--plan=p".to_string()])
    ...
    fn a_scale_target_decides_which_knob_a_multiple_turns(
        ...
        let arm = ArmSpec { multiple: 10.0, target, tighten_bps: 0.0 };
    ...
    fn a_unit_multiple_is_the_same_arm_whichever_target_it_names() {
        let unit = |target| ArmSpec { multiple: 1.0, target, tighten_bps: 0.0 };
    ...
    fn every_scale_target_crosses_the_multiples_and_shares_one_control() {
        let arms = scaled(&[1.0, 100.0], &[], &["all", "vaults", "ladder"])
            .arms()
            .expect("valid");
        assert_eq!(arms.len(), 4, "{:?}", arms.iter().map(ArmSpec::label).collect::<Vec<_>>());
        assert_eq!(arms.iter().filter(|arm| arm.is_control()).count(), 1);
        let labels = arms.iter().map(ArmSpec::label).collect::<Vec<_>>();
        assert_eq!(labels, ["1x", "100x", "100x ladder", "100x vaults"]);
    }
    ...
    fn an_accidental_cross_product_is_refused_before_it_runs_overnight() {
        ...
        let error = cli(&many, &bps).arms().expect_err("40 arms must be refused");
        assert!(error.to_string().contains("replays of the range"), "{error}");
    }

    #[rstest]
    #[case(ArmSpec::CONTROL, "1x")]
    #[case(ArmSpec { multiple: 100.0, target: ScaleTarget::All, tighten_bps: 0.0 }, "100x")]
    #[case(ArmSpec { multiple: 0.1, target: ScaleTarget::All, tighten_bps: 0.0 }, "0.1x")]
    #[case(ArmSpec { multiple: 100.0, target: ScaleTarget::Vaults, tighten_bps: 0.0 }, "100x vaults")]
    #[case(ArmSpec { multiple: 100.0, target: ScaleTarget::Ladder, tighten_bps: 0.0 }, "100x ladder")]
    #[case(ArmSpec { multiple: 10.0, target: ScaleTarget::All, tighten_bps: 2.5 }, "10x -2.5bps")]
    #[case(ArmSpec { multiple: 1.0, target: ScaleTarget::All, tighten_bps: 2.5 }, "-2.5bps")]
    fn an_arm_labels_itself_by_every_knob_it_turns(#[case] arm: ArmSpec, #[case] expected: &str) {
        assert_eq!(arm.label(), expected);
    }
}
