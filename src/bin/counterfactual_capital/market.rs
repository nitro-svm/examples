...
pub(crate) fn scale(
    ...
    for (mirror, amount) in layout.balance_mirrors.iter().zip(vault_amounts) {
        ...
        write_u128(&mut data, *mirror, U64, scaled(u128::from(*amount), capital)?)?;
    }

    let after = before
        .iter()
        .map(|ladder| Ladder {
            tiers: ladder.tiers.clone(),
        })
        .collect::<Vec<_>>();
    let after = tighten(after, tighten_bps);
    let after = after
        .into_iter()
        .map(|ladder| {
            Ok(Ladder {
...
fn scaled(value: u128, multiple: f64) -> Result<u128> {
    ...
    if scaled >= u128::MAX as f64 {
        bail!("scaling {value} by {multiple}x saturates, so this arm would hold less than it claims");
    }
...
mod tests {
    ...
    fn layout() -> StateLayout {
        StateLayout {
            account: "FQmFVQ7i8GCLqwE7EriUA4WQCvyCKxAeY4XUr55ZD6D7".parse().expect("an address"),
            discriminator: Some("tempest1".to_string()),
            len: 2385,
            max_tiers: 32,
            balance_mirrors: vec![2321, 2329],
            ladders: vec![
                LadderLayout { count: 209, entries: 225, stride: 32, width: 16 },
                LadderLayout { count: 1265, entries: 1281, stride: 32, width: 16 },
            ],
        }
    }
    ...
    fn state(vault_amounts: [u64; 2]) -> Account {
        ...
        Account {
            lamports: 17_490_480,
            data,
            owner: "2G71FYychcDbuNvMmVcFzU8J9N3giFvgp3a5PS2LmgZm".parse().expect("an address"),
            executable: false,
            rent_epoch: 0,
        }
    }
    ...
    fn scale_at(capital: f64, depth: f64, tighten_bps: f64) -> ScaledState {
        scale(&state(VAULTS), &layout(), &VAULTS, capital, depth, tighten_bps).expect("scales")
    }
    ...
    fn the_balance_mirrors_move_with_the_vaults() {
        ...
        for (mirror, amount) in layout().balance_mirrors.iter().zip(VAULTS) {
            assert_eq!(read_u128(&data, *mirror, U64).expect("in bounds"), u128::from(amount) * 10);
        }
    }
    ...
    fn a_venue_that_mirrors_no_balances_still_scales_its_ladder() {
        let bare = StateLayout { balance_mirrors: Vec::new(), ..layout() };
    ...
    fn a_spread_adjustment_without_two_sides_is_refused_rather_than_ignored() {
        let one = StateLayout { ladders: vec![layout().ladders[0].clone()], ..layout() };
