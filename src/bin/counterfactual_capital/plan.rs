...
mod tests {
    ...
    fn the_shipped_tempest_plan_parses_and_validates() {
        let path = concat!(
            ...
            "/src/bin/counterfactual_capital/tempest-sol-usdc.json"
        );
    ...
    #[test]
    fn a_read_only_account_in_the_venues_run_is_refused() {
        let mut plan = tempest();
        let vault = plan.inventory.vaults[0];
        plan.direct_fill
            .market
            .accounts
            .iter_mut()
            .filter(|entry| entry.address == vault)
            .for_each(|entry| entry.writable = false);
        let error = plan
            .validate()
            .expect_err("a read-only vault must be refused");
    ...
    fn a_vault_outside_the_account_run_is_refused() {
        let mut plan = tempest();
        plan.inventory.vaults[0] = "11111111111111111111111111111111"
            .parse()
            .expect("an address");
        let error = plan
            .validate()
            .expect_err("an unloaded vault must be refused");
    ...
    fn a_mirror_count_that_does_not_match_the_vaults_is_refused() {
        ...
        plan.inventory
            ...
            .as_mut()
            .expect("a ladder")
            .balance_mirrors = vec![2321];
        let error = plan
            .validate()
            .expect_err("a partial mirror list must be refused");
    ...
    fn a_tier_count_large_enough_to_overflow_the_bounds_check_is_refused() {
        ...
        let error = plan
            .validate()
            .expect_err("an overflowing bound must be refused");
        assert!(
            error.to_string().contains("runs past the account's"),
            "{error}"
        );
    }
    ...
    fn a_tier_count_offset_outside_the_account_is_refused() {
        ...
        let error = plan
            .validate()
            .expect_err("an out-of-bounds count offset must be refused");
    ...
    fn a_ladder_running_past_the_account_is_refused() {
        ...
        let error = plan
            .validate()
            .expect_err("an out-of-bounds ladder must be refused");
